...
(async () => {
  ...
  await page.setContent(`
    ...
    $3Dmol.download("pdb:1HVR", viewer, {doAssembly:false, noComputeSecondaryStructure: true}, function() {
      const m = viewer.getModel(0);
      const atoms = m.selectedAtoms({});
      
      atoms.forEach(a => {
         if (a.resi >= 1 && a.resi <= 20) a.ss = 'h';
         else if (a.resi >= 21 && a.resi <= 40) a.ss = 's';
         else a.ss = 'c';
      });
