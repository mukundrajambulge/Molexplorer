...
(async () => {
  ...
  await page.setContent(`
    ...
    $3Dmol.download("pdb:1HVR", viewer, {doAssembly:false}, function() {
      ...
      viewer.setStyle({ss: 'h'}, { cartoon: { style: 'oval', color: 'blue' } });
      viewer.setStyle({ss: 's'}, { cartoon: { style: 'edged', arrows: true, color: 'orange' } });
      viewer.setStyle({ss: 'c'}, { cartoon: { style: 'trace', color: 'gray' } });
      viewer.render();
      console.log("Rendered separate styles!");
    });
    </script>
    </body>
    </html>
