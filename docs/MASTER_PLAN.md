...
**Docking power** — A scoring function's ability to identify the correct (near-native) binding pose among many candidate poses.
...
**Screening power** — A scoring function's ability to distinguish true binders from non-binders (decoys) across a large library, relevant to virtual screening.
...
- Database schema changes go through versioned migration scripts (a simple numbered SQL migration folder is sufficient at this scale — no need for a heavyweight migration framework yet), so the schema's history is reconstructable and repeatable across environments.
