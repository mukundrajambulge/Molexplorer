...
When evaluating composite expressions, operators adhere to the following precedence hierarchy (ordered from highest to lowest binding strength):

| Precedence Tier | Operator / Syntax | Associativity | Description / Semantics | Classification |
|---|---|---|---|---|
| **1 (Highest)** | `( ... )` | None | Explicit grouping parentheses | `CURRENT_IMPLEMENTED` |
| **2** | Property selectors (`name`, `resn`, etc.), Comparisons (`b > 30`) | Left | Atomic property evaluation | `CURRENT_IMPLEMENTED` |
| **3** | Unary `not`, `!` | Right | Set complement ($U \setminus S$) | `CURRENT_IMPLEMENTED` |
| **4** | Topological (`byres`, `bychain`, `bymolecule`, `neighbor`, `extend`) | Right | Graph-based expansion modifiers | `CURRENT_IMPLEMENTED` |
| **5** | Spatial (`around`, `within`, `beyond`) | Right | Metric coordinate distance filters | `CURRENT_IMPLEMENTED` |
| **6** | Binary `and`, `&`, or whitespace juxtaposition | Left | Set intersection ($A \cap B$) | `CURRENT_IMPLEMENTED` |
| **7 (Lowest)** | Binary `or`, `\|` | Left | Set union ($A \cup B$) | `CURRENT_IMPLEMENTED` |

### 7.1 Juxtaposition (Implicit AND) Rule
...
The following table documents the complete property selector matrix:

| Selector | Accepted Syntax | Data Type | Matching Policy | Wildcard (`*`) | List (`+`) | Range (`-`) | Implementation Status | Evidence / Validation Reference |
|---|---|---|---|---|---|---|---|---|
| `name` | `name CA`, `name CA+CB` | String | Case-insensitive PDB atom name | **YES** (`name C*`) | **YES** (`CA+N+O`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:475`, `qa_group8_selection_query.ts` |
| `resn` | `resn ALA`, `resn LIG` | String | Case-insensitive 3-letter code | **YES** (`resn AL*`) | **YES** (`ALA+GLY`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:472`, `qa_group8_selection_query.ts` |
| `resi` | `resi 1-50`, `resi 10+20` | Integer / Range | Sequence number (`resSeq`) | **NO** | **YES** (`10+20`) | **YES** (`1-50`) | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:478`, `qa_group8_selection_query.ts` |
| `chain` | `chain A`, `chain A+B` | String | Alphanumeric chain ID | **YES** (`chain *`) | **YES** (`A+B+H`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:473`, `qa_group8_selection_query.ts` |
| `elem` | `elem C`, `elem FE` | String | Normalized element symbol | **YES** (`elem *`) | **YES** (`C+N+O`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:474`, `qa_group8_selection_query.ts` |
| `id` | `id 100`, `id 1-50` | Integer / Range | Canonical Atom ID (`serial`) | **NO** | **YES** (`1+2+3`) | **YES** (`1-100`) | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:479`, `qa_group8_selection_query.ts` |
| `b` | `b > 30.0`, `b <= 15.5` | Float64 | Comparison against B-factor | **NO** | **NO** | Numeric comparisons | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:501`, `qa_group8_selection_query.ts` |
| `q` | `q == 1.0`, `q < 0.5` | Float64 | Comparison against occupancy | **NO** | **NO** | Numeric comparisons | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:503`, `qa_group8_selection_query.ts` |
| `alt` | `alt A`, `alt B` | Character | Alternate location identifier | **NO** | **YES** (`A+B`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:476`, `test_selection_engine.ts` |
| `segi` | `segi PROT` | String | Segment identifier | **YES** (`segi *`) | **YES** (`A+B`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:476` (maps to altloc/segment fallback) |
| `ss` | `ss h`, `ss s`, `ss l` | String | Secondary structure class | **NO** | **YES** (`h+s`) | **NO** | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:481`, `validate_dssp_dihedrals.md` |
...
| Class Selector | Scientific Definition & Qualifying Criteria | Implementation Status | Evidence Reference |
|---|---|---|---|
| `all` | The entire universe of atoms in active query scope ($S = \mathcal{A}_{\text{scope}}$). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:610` |
| `none` | The empty set ($S = \emptyset$). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:614` |
| `polymer` | Residues belonging to continuous biological polymers (`polymer.protein` $\cup$ `polymer.nucleic`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:556` |
| `polymer.protein` | Canonical 20 amino acids + `MSE`, `SEC`, `PYL`, `HYP` (non-solvent). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:559` |
| `polymer.nucleic` | Canonical ribonucleotides (`A`, `C`, `G`, `U`) and deoxyribonucleotides (`DA`, `DC`, `DG`, `DT`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:562` |
| `organic` | Hetero-residues (`isHetero === true`), excluding solvent, containing at least one carbon atom. | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:550` |
| `inorganic` | Hetero-residues (`isHetero === true`), excluding solvent, containing zero carbon atoms. | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:553` |
| `solvent` | Water molecules and crystallographic solvent species (`HOH`, `WAT`, `DOD`, `TIP3`, `SOL`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:582` |
| `hetatm` | All atoms originating from PDB `HETATM` records or flagged as non-standard heterogens. | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:585` |
| `backbone` | Protein peptide backbone (`N`, `CA`, `C`, `O`, `OXT`, `H`) and nucleic acid backbone (`P`, `O3'`, `O5'`, `C3'`, `C4'`, `C5'`, `O4'`, `C1'`, `C2'`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:565` |
| `sidechain` | All atoms in amino acid or nucleic acid residues not belonging to the backbone. | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:571` |
| `guide` | C-alpha atoms (`CA`) in proteins and Phosphorus atoms (`P`) in nucleic acids. | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:577` |
| `hydrogens` | Hydrogen and deuterium isotopes (`elem === 'H' || elem === 'D'`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:588` |
| `metals` | Transition metals and biological metal ions (`MG`, `ZN`, `FE`, `CA`, `NA`, `K`, `CU`, `MN`, `NI`, `CO`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:591` |
| `donors` | Hydrogen-bond donors: hydrogens bonded to electronegative atoms, or electronegative atoms with available polar hydrogens. | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:594` |
| `acceptors` | Hydrogen-bond acceptors: electronegative atoms with lone pairs (`O`, `N`, `F`, `S`). | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:605` |
...
| Query Syntax | Normalized Match | Qualifying Structure Types | Implementation Status |
|---|---|---|---|
| `ss h`, `ss helix` | Helix | $\alpha$-helix ($4_{13}$), $3_{10}$-helix, $\pi$-helix | `CURRENT_IMPLEMENTED` |
| `ss s`, `ss sheet`, `ss strand`, `ss e` | Sheet | $\beta$-sheet strand, extended $\beta$-bridge | `CURRENT_IMPLEMENTED` |
| `ss l`, `ss loop`, `ss c`, `ss coil` | Loop / Coil | Irregular loops, turns, bends, flexible termini | `CURRENT_IMPLEMENTED` |
...
1. **`neighbor <selection>`:**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:355`).
...
2. **`extend <N> [of] <selection>`:**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:370`).
...
3. **`byres <selection>`:**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:300`).
...
4. **`bychain <selection>`:**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:312`).
...
5. **`bymolecule` / `byobject` `<selection>`:**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:323`).
...
1. **`within <cutoff> [of] <selection>` (Proximity Sphere):**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:416`).
...
2. **`around <cutoff> [of] <selection>` (Proximity Shell):**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:405`).
...
3. **`beyond <cutoff> [of] <selection>` (Exterior Void):**
   ...
   - **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:427`).
...
|---|---|---|---|
| **Active Object** | Single active `Object` | Query evaluates only across atoms in the currently selected object. | `CURRENT_IMPLEMENTED` |
| **Active Molecule** | Single `Molecule` hierarchy | Query evaluates across all chains and residues within the molecule. | `CURRENT_IMPLEMENTED` |
...
5. **Status:** `CURRENT_IMPLEMENTED` (`SelectionParser.ts:626`, `SessionSchema.ts:NamedSelectionSession`).
...
|---|---|---|---|
| **All Atoms** | `all` | Selects entire molecular universe. | `CURRENT_IMPLEMENTED` |
| **Protein Backbone** | `polymer.protein and backbone` | Selects protein peptide chain (`N, CA, C, O`). | `CURRENT_IMPLEMENTED` |
| **Sidechains** | `polymer.protein and sidechain` | Selects all amino acid sidechain functional groups. | `CURRENT_IMPLEMENTED` |
| **Ligands & Heteroatoms** | `organic or (hetatm and not solvent)` | Selects small-molecule ligands and cofactors. | `CURRENT_IMPLEMENTED` |
| **Water Molecules** | `solvent` (`resn HOH+WAT+DOD+SOL`) | Selects crystallographic water species. | `CURRENT_IMPLEMENTED` |
| **Alpha Helices** | `ss h` | Selects helical protein segments. | `CURRENT_IMPLEMENTED` |
| **Beta Sheets** | `ss s` | Selects extended $\beta$-strand sheets. | `CURRENT_IMPLEMENTED` |
| **Binding Pocket** | `byres (around 5.0 of organic)` | Selects all residues within 5.0 Å of any ligand. | `CURRENT_IMPLEMENTED` |
...
## 34. Golden Test Fixtures

The platform establishes the following verified golden test fixture suite:

| Fixture ID | Structure Name / Description | Verified Atom Count | Key Selection Benchmarks | Evidence Reference |
|---|---|---|---|---|
| `01_simple_protein.pdb` | Single-chain synthetic peptide | 20 atoms | `all` $\to$ 20, `none` $\to$ 0, `name CA` $\to$ 5 | `MASTER_PLAN.md:278`, `qa_group8` |
| `02_two_chain_protein.pdb`| Two-chain dimeric protein | 24 atoms | `chain A` $\to$ 12, `chain B` $\to$ 12 | `MASTER_PLAN.md:588`, `qa_group8` |
| `03_protein_with_ligand.pdb`| Protein + 4-atom organic ligand | 20 atoms | `hetatm and not resn HOH` $\to$ 4, `polymer` $\to$ 16 | `MASTER_PLAN.md:304`, `test_phase4` |
| `05_water_and_heteroatoms.pdb`| Protein + ions + waters | 25 atoms | `resn HOH` $\to$ 5, `metals` $\to$ 2 | `MASTER_PLAN.md:590`, `qa_group8` |
| `1CRN.pdb` | Crambin (Small Plant Seed Protein) | 327 atoms | `ss h` $\to$ 152, `ss s` $\to$ 48, `resn THR` $\to$ 28 | `qa_group8_selection_query.ts` |
| `1HVR.pdb` | HIV-1 Protease + XK263 Inhibitor | 1582 atoms | `organic` $\to$ 41, `solvent` $\to$ 128, `byres (around 5.0 of organic)` | `qa_group8_selection_query.ts` |
| `1BNA.pdb` | Synthetic B-DNA Dodecamer | 486 atoms | `polymer.nucleic` $\to$ 486, `guide` $\to$ 24 (P atoms) | `qa_group8_selection_query.ts` |
| `4HHB.pdb` | Deoxy Human Hemoglobin Tetramer | 4376 atoms | `chain A+B+C+D`, `organic` (Heme groups), `altLoc` disorder | `qa_group8_selection_query.ts` |
...
| Capability | Current File | Current Syntax | Status | Evidence Reference | Gap / Future Refactor Requirement |
|---|---|---|---|---|---|
| **Atom Name Matching** | `src/lib/SelectionParser.ts:475` | `name CA+CB` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:59` | Add support for padded 4-character exact PDB name matching. |
| **Residue Name Matching** | `src/lib/SelectionParser.ts:472` | `resn ALA+GLY` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:65` | Full conformance verified. |
| **Residue Sequence Ranges** | `src/lib/SelectionParser.ts:478` | `resi 1-50` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:71` | Extend parser to handle insertion codes (`resi 100A-105`). |
| **Chain ID Matching** | `src/lib/SelectionParser.ts:473` | `chain A+B` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:77` | Support blank chain selector (`chain ''`). |
| **Element Matching** | `src/lib/SelectionParser.ts:474` | `elem C+N+O` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:83` | Full conformance verified. |
| **B-factor Comparison** | `src/lib/SelectionParser.ts:501` | `b > 30.0` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:277` | Full conformance verified. |
| **Occupancy Comparison** | `src/lib/SelectionParser.ts:503` | `q < 1.0` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:277` | Full conformance verified. |
| **Secondary Structure** | `src/lib/SelectionParser.ts:481` | `ss h`, `ss s` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:89` | Connect to backend 3DNA nucleic secondary structure. |
| **Boolean Operators** | `src/lib/SelectionParser.ts:114` | `and`, `or`, `not` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:104` | Full precedence conformance verified. |
| **Parentheses Grouping** | `src/lib/SelectionParser.ts:156` | `(...)` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:158` | Full conformance verified. |
| **Spatial Proximity (within)** | `src/lib/SelectionParser.ts:415` | `within 4.0 of ...` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:123` | Accelerate via SpatialHashGrid candidate pruning. |
| **Spatial Shell (around)** | `src/lib/SelectionParser.ts:404` | `around 4.0 of ...` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:132` | Verified self-exclusion behavior. |
| **Topological Expansion (byres)** | `src/lib/SelectionParser.ts:299` | `byres (...)` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:141` | Full conformance verified. |
| **Topological Expansion (bychain)**| `src/lib/SelectionParser.ts:311` | `bychain (...)` | `CURRENT_IMPLEMENTED` | `test_selection_engine.ts:64` | Full conformance verified. |
| **Topological Graph (neighbor)** | `src/lib/SelectionParser.ts:354` | `neighbor (...)` | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:354` | Migrate from array index bonds to canonical ID bonds. |
| **Topological Graph (extend)** | `src/lib/SelectionParser.ts:369` | `extend 2 of ...` | `CURRENT_IMPLEMENTED` | `SelectionParser.ts:369` | Migrate to canonical ID graph edge list. |
| **Chemical Classes (organic, etc.)**| `src/lib/SelectionParser.ts:525` | `organic`, `metals` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:98` | Standardized definitions verified. |
| **Named Selection Storage** | `src/lib/SelectionParser.ts:626` | `select name, expr` | `CURRENT_IMPLEMENTED` | `qa_group8_selection_query.ts:183` | Full session round-trip verified. |
...
- [x] Complete property selector matrix and chemical class definitions are classified with repository evidence.
...
- [x] Multi-state and altLoc disorder conformer selections are defined without overriding P0.2.
...
- [x] Complete codebase audit matrix and verified manual golden fixtures are included.
