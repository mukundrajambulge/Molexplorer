...
function getColorFunction(
  ...
  return (atom: any): string => {
    ...
    if (csLower === 'by formal charge') {
      ...
      if (charge < 0) return '#ef4444'; // Red
      if (charge > 0) return '#3b82f6'; // Blue
      return '#ffffff';
    }

    if (csLower === 'by partial charge' || csLower === 'esp') {
      const elem = (atom.elem || '').toUpperCase();
      if (elem === 'O' || elem === 'F' || elem === 'CL') return '#ef4444'; // Negative: Red
      if (elem === 'N' || elem === 'H') return '#3b82f6'; // Positive: Blue
      return '#f3f4f6';
    }
    ...
    if (csLower === 'hydrophobicity') {
      ...
      if (hydrophobic.includes(resn)) return '#eab308'; // Hydrophobic: Gold
      return '#3b82f6'; // Polar: Blue
    }
...
export default forwardRef<MolStudioViewerRef, {
  ...
  pdbData, ssData, ssMode, assemblyPDB, symmetryPDB, alignmentPDB, ligandData,
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (pdbData) {
        ...
        // Base style for protein/nucleic polymers
        setClickStyle({ hetflag: false }, getStyleObj(renderStyle, colorScheme, minResi, maxResi, chainMap, 1.0));

        // Base style for organic ligands/inhibitors (non-water HETATMs)
        ...
        if (selectedAtomSerials.size > 0) {
          setClickStyle({ hetflag: false }, getStyleObj(renderStyle, 'white', minResi, maxResi, chainMap, 0.25));
  ...
  }, [pdbData, ssData, ssMode, assemblyPDB, symmetryPDB, alignmentPDB, ligandData, interactions, renderStyle, colorScheme, surfaceOpacity, selectedAtomSerials, backgroundColor, focusTrigger, measurements, activeMeasurementMode, showDipoleArrow, dipoleMoment]);
