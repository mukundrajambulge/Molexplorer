...
export default function Toolbar({ viewState, onViewStateChange }: ToolbarProps) {
  ...
  return (
    <div className="min-h-[3.5rem] border-b border-white/10 bg-[#0A0A0A]/90 backdrop-blur-md flex items-center px-3 sm:px-6 py-2 gap-2 sm:gap-4 text-[10px] font-mono tracking-[0.08em] uppercase z-10 flex-wrap overflow-x-auto no-scrollbar whitespace-nowrap">
      
      {/* Navigation Switchers */}
      <div className="flex items-center gap-1.5 font-sans normal-case">
        <a 
          href="/" 
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-white/10 bg-white/[0.04] text-slate-300 hover:text-white hover:bg-white/10 transition-all text-xs font-medium cursor-pointer"
        ...
        <a 
          href="/molstudio" 
          className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-300 hover:bg-amber-500/20 transition-all text-xs font-medium cursor-pointer"
          title="Open MolStudio 3D"
        >
          ⚡ <span>MolStudio</span>
        </a>
      </div>
