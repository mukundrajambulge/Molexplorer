 ...
 * Tokenizer, literals, parentheses, logical precedence, comparisons, ranges, lists,
 * wildcards, identity (id vs index vs rank), classifications, topology (neighbor != bound_to),
 * spatial (within != expand != around), hierarchy, named selections, multi-object/state scope,
 * malformed syntax, scientific immutability, independent oracle, property-based invariants,
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(tokens.length >= 8, 'Tokenized complex selection query into distinct tokens');
  assert(tokens.includes('byres') && tokens.includes('around') && tokens.includes('4.5'), 'Extracted keywords and numeric literals correctly');

  const macroTokens = parser.tokenize('//A/10/CA and ///10-25/ and /4DJW//A/C*');
  assert(macroTokens.length >= 5, 'Tokenized PyMOL slash macros correctly');
}

// ---------------------------------------------------------------------------------
// 2. IDENTITY DISTINCTIONS (id vs index vs rank)
// ---------------------------------------------------------------------------------
console.log('\n--------------------------------------------------------------------------------');
console.log('2. Identity Distinctions (id vs index vs rank)');
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(idRes.count === 1 && idRes.selected_ids.has(1), 'id 1 selected serial 1');

  // index = 0-based runtime offset
  const idxRes = evaluator.evaluateQuery('index 0');
  assert(idxRes.count === 1 && idxRes.selected_ids.has(1), 'index 0 selected first atom (0-based)');

  // rank = 1-based load order index
  const rankRes = evaluator.evaluateQuery('rank 1');
  assert(rankRes.count === 1 && rankRes.selected_ids.has(1), 'rank 1 selected first atom (1-based)');

  // Range on index: index 0-4
  const idxRange = evaluator.evaluateQuery('index 0-4');
  assert(idxRange.count === 5, 'index 0-4 selected exactly 5 atoms');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(nameRes.count === atomRes.count && nameRes.count === 46, 'name CA and atom CA alias parity (46 alpha carbons)');

  // Wildcard: name C*
  const wildRes = evaluator.evaluateQuery('name C*');
  assert(wildRes.count > nameRes.count, `name C* wildcard matched ${wildRes.count} atoms (CA, CB, CG, CD, etc.)`);
  ...
  assert(elemRes.count === elRes.count && elRes.count === symRes.count && elemRes.count === 6, 'elem/element/symbol S parity (6 sulfur atoms in 1CRN)');
  ...
  assert(r1.count === r2.count && r2.count === r3.count && r1.count > 0, 'resi/resv range syntax parity (10-20 == 10:20)');

  // Lists: resn ALA+GLY+CYS and resn ALA,GLY,CYS
  const listPlus = evaluator.evaluateQuery('resn ALA+GLY+CYS');
  const listComma = evaluator.evaluateQuery('resn ALA,GLY,CYS');
  assert(listPlus.count === listComma.count && listPlus.count > 0, 'resn list syntax parity (ALA+GLY+CYS == ALA,GLY,CYS)');

  // Comparisons on b and q
  const bRes = evaluator.evaluateQuery('b < 10.0');
  const qRes = evaluator.evaluateQuery('q == 1.0');
  assert(bRes.count >= 0 && qRes.count === 327, 'b-factor and occupancy comparison predicates evaluated cleanly');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(polyRes.count === 16, 'polymer matched 16 protein atoms');
  assert(protRes.count === 16, 'protein alias matched 16 protein atoms');
  assert(orgRes.count === 4, 'organic matched 4 ligand atoms');
  assert(bbRes.count + scRes.count === polyRes.count, 'backbone + sidechain partitions polymer exactly (16 atoms)');

  // first and last ordinals
  const firstRes = evaluator.evaluateQuery('first');
  const lastRes = evaluator.evaluateQuery('last');
  assert(firstRes.count === 1 && firstRes.selected_ids.has(1), 'first matched serial 1');
  assert(lastRes.count === 1 && lastRes.selected_ids.has(20), 'last matched serial 20');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(nRes.count > 0, `neighbor id 2 returned ${nRes.count} bonded atom(s)`);
  assert(!nRes.selected_ids.has(2), 'neighbor strictly excludes the query atom (id 2)');
  assert(bRes.count >= nRes.count, 'bound_to includes incident connectivity');

  // extend 2 of id 1
  const ext1 = evaluator.evaluateQuery('extend 1 of id 1');
  const ext2 = evaluator.evaluateQuery('extend 2 of id 1');
  assert(ext2.count >= ext1.count, `extend 2 (${ext2.count}) >= extend 1 (${ext1.count}) topological radius expansion`);
  ...
  assert(byresRes.count === 5, `byres id 1 expanded to entire residue (${byresRes.count} atoms)`);
  assert(bychainRes.count === 20, `bychain id 1 expanded to chain (${bychainRes.count} atoms)`);
  assert(bymolRes.count === 16, `bymolecule id 1 expanded to connected component (${bymolRes.count} atoms, excluding non-bonded ligand)`);
  assert(bycaRes.count === 1, 'bycalpha id 1 isolated the single CA atom in residue 1');
  ...
  assert(threwFragment, 'byfragment fails closed with explicit DEFERRED notice');

  let threwCell = false;
  try { evaluator.evaluateQuery('bycell all'); } catch (e: any) {
    threwCell = e.message.includes('DEFERRED');
  }
  assert(threwCell, 'bycell fails closed with explicit DEFERRED notice');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(isSubset, 'Invariant verified: S ⊆ expand(d, S)');
  ...
  assert(!aroundIntersectsS, 'Invariant verified: around(d, S) strictly excludes S');
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(p1.count === p2.count && p1.count === 4, 'byres weak precedence verified: "byres resn LIG and chain A" == "byres (resn LIG and chain A)"');

  // Logical precedence: not > and > or
  const boolRes = evaluator.evaluateQuery('chain A or not resn LIG and name CA');
  assert(boolRes.count > 0, 'Evaluated complex boolean expression respecting operator precedence');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(m1.count === m1Ref.count && m1.count === 1, 'Slash macro //A/10/CA matched exactly 1 atom identically to canonical query');

  // ///1-10/ -> resi 1-10
  const m2 = evaluator.evaluateQuery('///1-10/');
  const m2Ref = evaluator.evaluateQuery('resi 1-10');
  assert(m2.count === m2Ref.count && m2.count > 0, 'Slash macro ///1-10/ matched residue range 1-10 identically');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(ligRef.count === 4, 'Named selection "ligand" resolved correctly');

  const compRes = evaluator.evaluateQuery('byres (ligand around 3.5) and not ligand');
  assert(compRes.count >= 0, 'Composed named selection within complex nested spatial query');
  ...
  assert(threwUnknown, 'Unknown selection reference throws fail-closed error');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(actRes.total_count === 20, 'Document active_object scope evaluated exactly 20 atoms');

  // Scoped atom keys format: "object_id:canonical_id"
  const expectedKey = `${doc.active_object_id}:1`;
  assert(actRes.scoped_keys.has(expectedKey), `Scoped key format verified (${expectedKey})`);
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(queryRes.count === oracleSet.size, `Oracle count parity: ${queryRes.count} == ${oracleSet.size}`);
  let oracleMatch = true;
  for (const id of oracleSet) {
    if (!queryRes.selected_ids.has(id)) oracleMatch = false;
  }
  assert(oracleMatch, 'Independent oracle atom-for-atom agreement (SCIENTIFICALLY / RULE-BASED VALIDATED)');
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(allRes.count === mol.atoms.length, 'evaluate("all") == Universe');

  // evaluate("none") == empty
  const noneRes = evaluator.evaluateQuery('none');
  assert(noneRes.count === 0, 'evaluate("none") == empty');

  // not(not(S)) == S
  const sTarget = evaluator.evaluateQuery('name P');
  const doubleNot = evaluator.evaluateQuery('not (not (name P))');
  assert(doubleNot.count === sTarget.count, 'Double complement invariant: not(not(S)) == S');

  // S OR S == S
  const idempotenceOr = evaluator.evaluateQuery('name P or name P');
  assert(idempotenceOr.count === sTarget.count, 'Idempotence invariant: S OR S == S');

  // S AND S == S
  const idempotenceAnd = evaluator.evaluateQuery('name P and name P');
  assert(idempotenceAnd.count === sTarget.count, 'Idempotence invariant: S AND S == S');

  // S ∩ not(S) == empty
  const disjRes = evaluator.evaluateQuery('name P and not (name P)');
  assert(disjRes.count === 0, 'Disjoint invariant: S ∩ not(S) == ∅');
}
...
for (const fix of fixtures) {
  ...
  assert(u.count === mol.atoms.length, `Fixture ${fix}: Universe invariant holding (${u.count} == ${mol.atoms.length})`);

  // Discovered chain
  const firstChain = mol.chains[0]?.name || mol.chains[0]?.chain_id || 'A';
  const chRes = evaluator.evaluateQuery(`chain ${firstChain}`);
  assert(chRes.count > 0, `Fixture ${fix}: Dynamic chain discovery "chain ${firstChain}" selected ${chRes.count} atoms`);
}
...
console.log('--------------------------------------------------------------------------------');
{
  ...
  assert(hashBefore === hashAfter, `Read-only invariant verified: H(before) == H(after) (${hashBefore})`);
}
