...
(async () => {
  console.log('============================================================');
  console.log('    TASK P3.3: MANUAL BROWSER VERIFICATION (BOND ORDER)     ');
  console.log('============================================================\n');
  ...
  const shot1 = path.join(SCREENSHOT_DIR, 'order_01_baseline.png');
  await page.screenshot({ path: shot1 });
  console.log(`  -> Saved screenshot: ${shot1}`);

  // Step 3 & 4: Execute bond order modification: "order id 17, id 20, 2"
  console.log('4. Executing Command: "order id 17, id 20, 2"...');
  const orderRes = await page.evaluate(() => {
    return window.__molStudioTestApi.runQuery('order id 17, id 20, 2');
  });
  console.log(`  -> Order Output: ${orderRes.textOutput}`);

  await new Promise(r => setTimeout(r, 1500));

  const shot2 = path.join(SCREENSHOT_DIR, 'order_02_after_order2.png');
  await page.screenshot({ path: shot2 });
  console.log(`  -> Saved screenshot: ${shot2}`);

  // Step 5: Cycle bond order on (17, 18)
  console.log('5. Executing Command: "cycle_valence id 17, id 18"...');
  const cycleRes = await page.evaluate(() => {
    return window.__molStudioTestApi.runQuery('cycle_valence id 17, id 18');
  });
  console.log(`  -> Cycle Output: ${cycleRes.textOutput}`);

  await new Promise(r => setTimeout(r, 1500));

  const shot3 = path.join(SCREENSHOT_DIR, 'order_03_after_cycle.png');
  await page.screenshot({ path: shot3 });
  console.log(`  -> Saved screenshot: ${shot3}`);

  // Step 6: Rotate and adjust 3D camera
  ...
  // Step 7: Export MolStudio-PSE session string
  ...
  // Step 8: Reload session and verify topology persistence
  ...
  const shot4 = path.join(SCREENSHOT_DIR, 'order_04_reloaded_state.png');
