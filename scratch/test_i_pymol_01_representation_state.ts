...
console.log('--------------------------------------------------------------------------------');
{
  ...
  // Atom C (atom 3) is positioned at distance 1.10 � from atom 2 (within covalent bond threshold 1.5 �)
  ...
  // Verify distance between Atom 2 and Atom 3 is 1.1 � (geometrically close)
  const d23 = Math.sqrt((proc.atoms[2].x - proc.atoms[1].x)**2 + (proc.atoms[2].y - proc.atoms[1].y)**2 + (proc.atoms[2].z - proc.atoms[1].z)**2);
  assert(Math.abs(d23 - 1.1) < 1e-3, `Atom 2 and Atom 3 are geometrically close (d = ${d23.toFixed(2)} �)`, 'SCIENTIFICALLY VALIDATED');
